...
const Profile = (props) => {
  ...
  const thisUser = user.map((userData, index) => {
    return (
      <div className="profile-container">
        <div>
          {userData.photo ? (
            <img className="profile-pic" src={userData.photo} />
          ) : (
            <img
              className="profile-pic"
              src="https://res.cloudinary.com/ddmbb2ian/image/upload/v1615672962/qvo_UWEYzvsVDmwUPEWLsCIh_xjgub8.jpg"
            />
          )}
  ...
  return <div className="text-center pt-4">{thisUser}</div>;
};
